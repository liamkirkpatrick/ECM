...
- Live plotting of AC runs
- "BID Mode", which can be turned off to speed up running.

Requires functions:
  -laser_v2
  -makegui_v10
  -setup_movement_v3
  -ac_live_plot_subproccess.py
  -ac_live_plot_worker.py
"""
...
from ac_live_plot_subprocess import ACSubprocessLivePlot
...
def main():
    ...
    # Separate-process AC live plot handle.
    # This is only used for AC because DC data are buffered on the SMU until each track ends.
    ac_live_plot = None
    ...
    if qt:
        ...
        while True:
            ...
            if event == '-NEW-' or keyboard.is_pressed('Enter'):
                ...
                repeat = False
                break
            elif event == '-SAME-' or keyboard.is_pressed('r'):
                set_new = False
                flip = False
                repeat = True
                break
            elif event == '-FLIP-' or keyboard.is_pressed('s'):
                set_new = False
                flip = True
                repeat = True
                break
    ...
    if qt and AC_run:
        ...
        while True:
            ...
            if event == '-START_AC-' or keyboard.is_pressed('Enter'):
                ...
                # Start a separate-process AC live plot. The acquisition loop only
                # pushes tiny messages into a queue; matplotlib redraws happen in
                # the worker process so they should not slow encoder polling.
                try:
                    ac_distance = np.arange(0,int(countmax)) * c.mm_per_step
                    ac_live_plot = ACSubprocessLivePlot(
                        ydim,
                        ac_distance,
                        title='AC Runs - Live',
                        initial_ylim=(0, 10e-8),
                        update_interval=0.25,
                        send_every_points=5,
                    )
                except Exception as e:
                    ac_live_plot = None
                    print('WARNING - Failed to start AC live plot process:', repr(e))
                ...
                for y in np.arange(0,len(ydim)):
                    ...
                    while cnt < countmax:
                        ...
                        if state != stateprior:
                            ...
                            # Queue a small live-plot update. This does not redraw
                            # matplotlib and drops messages rather than blocking
                            # the encoder-critical loop.
                            if ac_live_plot is not None:
                                ac_live_plot.send_point(
                                    int(y),
                                    int(cnt-1),
                                    (cnt-1) * c.mm_per_step,
                                    G_AC[cnt-1,y]
                    ...
                    # Send the completed track to the live plot and autoscale y
                    # once per track, outside the encoder-critical loop.
                    if ac_live_plot is not None:
                        ac_live_plot.finish_track(
                            int(y),
    ...
    # Close separate AC live plot worker, if it was started.
    try:
        if ac_live_plot is not None:
            ac_live_plot.close()
    except:
        print('Failed to close AC live plot process')
