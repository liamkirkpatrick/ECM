...
def guibuild():
    ...
    second_col = [
        ...
        [sg.Button('Set Z Down', size=(15, 1), key='-Z_SETUP-', disabled=True), sg.Button('Back (b)', key='-SETUP_BACK-', disabled=True), sg.Text('     Set At: '), sg.Text('not set', size=(8, 1), key='-Z_SET-')],
        ...
        [sg.Text()],
        [sg.Text('           '), sg.RealtimeButton(sg.SYMBOL_UP, key='-X_UP-'), sg.Text('           '), sg.RealtimeButton(sg.SYMBOL_UP, key='-Z_UP-')],
        [sg.RealtimeButton(sg.SYMBOL_LEFT, key='-Y_DOWN-'), sg.Text(size=(10, 1), key='-STATUS-', justification='c', pad=(0, 0)), sg.RealtimeButton(sg.SYMBOL_RIGHT, key='-Y_UP-')],
        [sg.Text('           '), sg.RealtimeButton(sg.SYMBOL_DOWN, key='-X_DOWN-'), sg.Text('           '), sg.RealtimeButton(sg.SYMBOL_DOWN, key='-Z_DOWN-')],
        [sg.Text()],
        [sg.Text('Enter Relative or Absolute Movements in mm')],
        [sg.Text()],
        [sg.Text('Move X Rel', size=(10, 1)), sg.InputText(size=(6, 1), key='X_rel'), sg.Text('Move X Abs', size=(10, 1)), sg.InputText(size=(6, 1), key='X_abs')],
    ...
    admin_col = [
        [sg.Text('STATUS:', font=("Helvetica", 12, "bold")), sg.Text('', size=(52, 1), key='-STEP_STATUS-', justification='left')],
        [sg.Text('')],
        [sg.Text('')],
        [sg.Column([[sg.Text('Admin Window', font=("Helvetica", 12, "bold"))]], justification='c')],
        [sg.Text('')],
        [sg.Button('New Section (enter)', key='-NEW-', disabled=True), sg.Button('Repeat Last (r)', key='-SAME-', disabled=True), sg.Button('Switch AC/DC (s)', key='-FLIP-', disabled=True), sg.Button('Back (b)', key='-ADMIN_BACK-', disabled=True)],
        [sg.Text("Enter tube:", size=(23, 1)), sg.InputText('', size=(10, 1), key="tube"), sg.Button('Submit', key='-SUBMIT_TUBE-', disabled=True)],
        [sg.Text("Enter index mark relative location:", size=(23, 1)), sg.InputText('', size=(10, 1), key="tieloc"), sg.Button('Submit', key='-SUBMIT_TIELOC-', disabled=True)],
        [sg.Text("Enter index mark depth:", size=(23, 1)), sg.InputText('', size=(10, 1), key="depth"), sg.Button('Submit', key='-SUBMIT_DEPTH-', disabled=True)],
